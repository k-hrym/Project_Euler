...
# 下の 20×20 の格子のうち, 上下左右斜めのいずれかの方向で連続する4つの数字の積のうち最大のものはいくつか?
...
# 縦
y = 0
while y < nest_num[0].size
  x = 0
  while x+3 < nest_num.size
    if max_inj < (x..x+3).map{|x| nest_num[x][y]}.inject(:*)
      max_inj = (x..x+3).map{|x| nest_num[x][y]}.inject(:*)
    end
    x += 1
  end
  y += 1
...
while x < nest_num.size
  y = 0
  while y+3 < nest_num[x].size
    if max_inj < nest_num[x][y..y+3].inject(:*)
      max_inj = nest_num[x][y..y+3].inject(:*)
    end
    y += 1
  end
  x += 1
...
# 左斜め
x = 0
while x+3 < nest_num[x].size
  y = 0
  while y+3 < nest_num[0].size
